...
export const generateInvoiceBuffer = async (order, company, logoUrl) => {
  let logoBuffer = null;
  try {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fontSize(10).text("Bill To:", leftColX, y, { underline: true });
      let addrY = y + 16;
      (order.addressLines || []).forEach((line) => {
        doc.text(line, leftColX, addrY);
        addrY += 12;
      });
      doc.fontSize(10).text("Invoice Details:", rightColX, y, { underline: true });
      let metaY = y + 16;
      doc.fontSize(9);
      ...
      for (const item of order.items) {
        ...
        let ix = tableLeft;
        doc.text(item.name || "", ix + 4, textY, {
          width: colDefs[0].width - 8,
        });
        ix += colDefs[0].width;
        doc.text(item.sku || "-", ix + 4, textY, {
          width: colDefs[1].width - 8,
        });
        ix += colDefs[1].width;
        doc.text(String(item.quantity || 1), ix, textY, {
        ...
        ix += colDefs[3].width;
        const sub = Number(
          item.subTotal ?? item.quantity * (item.salePrice ?? item.price ?? 0)
        );
        computedTotal += sub;
      ...
      const totalsX = tableLeft + tableWidth * 0.55;
      let totalsY = rowY + 15;
      ...
      const lineGap = 18;
      let ty = totalsY + 10;
      const subtotalValue = Number(order.totalAmount ?? computedTotal);
      const deliveryFee = Number(order.deliveryFee || 0);
      const grandTotal = subtotalValue + deliveryFee;
      doc.fontSize(10);
      doc.text("Subtotal:", totalsX + 10, ty, { width: boxW - 20 });
      doc.text(`${INR}${subtotalValue.toLocaleString()}`, totalsX + 10, ty, {
        align: "right",
        width: boxW - 20,
      });
      ty += lineGap;
      doc.text("Delivery Fee:", totalsX + 10, ty, { width: boxW - 20 });
      ...
      doc.text("Grand Total:", totalsX + 10, ty, { width: boxW - 20 });
      ...
      doc.fontSize(8).text(order.notes || "This is an electronically generated invoice.", startX, footY + 16, {
        width: usableWidth,
      });
      doc.fontSize(8).text(
